<x-public-layout>
    <!-- Header -->
    <div class="bg-slate-900 border-b border-slate-800 py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-black heading-font text-white">Katalog Buku Publik</h1>
            <p class="mt-2 text-sm text-slate-400">Temukan buku-buku terbaik dari berbagai kategori yang tersedia di perpustakaan kami.</p>
        </div>
    </div>
        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="w-full md:w-64 shrink-0 space-y-6">
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-5">
                    ...
                    <form action="{{ route('books.catalog') }}" method="GET" class="space-y-4">
                        <div>
                        ...
                        @if(request('search') || request('category'))
                            <a href="{{ route('books.catalog') }}" class="w-full flex items-center justify-center px-4 py-2 border border-gray-300 dark:border-slate-600 shadow-sm text-sm font-medium rounded-xl text-gray-700 dark:text-gray-300 bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                                Reset
                                </a>
                        @endif
                    </form>
