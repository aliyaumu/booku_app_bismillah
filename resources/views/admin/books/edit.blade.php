<x-admin-layout>
    ...
    <div class="max-w-4xl">
        <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 sm:p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" required class="block w-full rounded-xl border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm py-2.5 transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="author" id="author" value="{{ old('author', $book->author) }}" required class="block w-full rounded-xl border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm py-2.5 transition-colors">
                    </div>

                    <!-- Penerbit -->
                    <div>
                        <label for="publisher" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Penerbit</label>
                        <input type="text" name="publisher" id="publisher" value="{{ old('publisher', $book->publisher) }}" class="block w-full rounded-xl border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm py-2.5 transition-colors">
                    </div>

                    <!-- ISBN -->
                    <div>
                        <label for="isbn" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">ISBN</label>
                        <input type="text" name="isbn" id="isbn" value="{{ old('isbn', $book->isbn) }}" class="block w-full rounded-xl border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm py-2.5 transition-colors">
                    </div>

                    <!-- Tahun Terbit -->
                    <div>
                        <label for="published_year" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Tahun Terbit</label>
                        <input type="number" name="published_year" id="published_year" value="{{ old('published_year', $book->published_year) }}" min="1000" max="{{ date('Y') + 1 }}" class="block w-full rounded-xl border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm py-2.5 transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="total_stock" id="total_stock" value="{{ old('total_stock', $book->total_stock) }}" min="0" required class="block w-full rounded-xl border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm py-2.5 transition-colors">
                        <p class="mt-1.5 text-xs text-gray-500 dark:text-slate-400">
                            Stok tersedia saat ini: <span class="font-bold text-emerald-600 dark:text-emerald-400">{{ $book->available_stock }}</span>. Menurunkan stok total akan memotong stok tersedia.
                        </p>
                    </div>
                        ...
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-slate-600 border-dashed rounded-xl bg-gray-50 dark:bg-slate-900/50 hover:bg-gray-100 dark:hover:bg-slate-900 transition-colors group">
                            <div class="space-y-1 text-center">
                                ...
                                <p class="text-xs text-gray-500 dark:text-slate-500">
                                    Biarkan kosong jika tidak ingin mengubah cover. (PNG, JPG up to 2MB)
                                </p>
                            </div>
                        </div>
                    </div>
